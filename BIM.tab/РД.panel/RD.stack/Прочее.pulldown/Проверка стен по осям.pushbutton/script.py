...
from Autodesk.Revit.DB import Group, ViewSchedule, CopyPasteOptions, ElementTransformUtils, XYZ, UnitUtils, DisplayUnitType, Line, Reference, ReferenceArray, FilteredElementCollector, DimensionType, Options, Transaction, TransactionGroup, ElementId, Wall, ElementIntersectsSolidFilter,ElementIntersectsElementFilter, Solid, SetComparisonResult, CylindricalFace, Grid, CategoryType, LogicalOrFilter, ElementFilter, ElementCategoryFilter, DetailLine
...
class CashWall:
	def __init__(self, obj):
		...
		param1 = obj.LookupParameter('Пров_Ортогональность Осям')
		param2 = obj.LookupParameter('Пров_Ровно от Оси')
		
		if (param1 is None) or (param2 is None):
			fileName = r'W:\BIM-Ресурсы\Revit - 2 Стандарты\!ФОП\Параметры проекта '+app.VersionNumber+'.rvt'
			docFamily = app.OpenDocumentFile(fileName)

			schedules  = [x for x in FilteredElementCollector(docFamily).OfClass(ViewSchedule).ToElements() if x.Name == '111 - Проверка на Ровность Стен']
			...
			schedules  = [x for x in FilteredElementCollector(doc).OfClass(ViewSchedule).ToElements() if x.Name == '111 - Проверка на Ровность Стен']
			ids = List[ElementId]([x.Id for x in schedules])
			doc.Delete(ids)

			docFamily.Close(saveModified = False)

			schedules  = [x for x in FilteredElementCollector(doc).OfClass(ViewSchedule).ToElements() if x.Name.find('111 - Проверка на Ровность Стен')>=0]
...
for grid in selectionGrids:
 	...
 	for wall in selectionWalls:
		...
		if wall not in checklist_1:
			if Utils.isNormal(grid.orientation, wall.orientation):
				...
				try:				
					wall.object.LookupParameter('Пров_Ортогональность Осям').Set('Да')
				except:
					pass
			else:
				try:				
					wall.object.LookupParameter('Пров_Ортогональность Осям').Set('Нет')
				except:
					pass
	...
	for wall in grid.parallelWalls:
		...
		distance = abs((grid.end.Y - grid.start.Y)*point.X - (grid.end.X - grid.start.X)*point.Y + grid.end.X*grid.start.Y - grid.end.Y*grid.start.X)/math.sqrt((grid.end.Y - grid.start.Y)**2 + (grid.end.X - grid.start.X)**2)
		...
		if wall not in grid.distanceValidWalls:
			...
			if (round(distance, 5))%scale > 0:
				try:
					wall.object.LookupParameter('Пров_Ровно от Оси').Set('Нет')
				except:
					pass
				...
				try:
					wall.object.LookupParameter('Пров_Ровно от Оси').Set('Да')
				except:
					pass
