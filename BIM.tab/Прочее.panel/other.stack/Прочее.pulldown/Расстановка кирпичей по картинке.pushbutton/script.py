...
familyCollector.OfClass(FamilySymbol)
familyList = familyCollector.WhereElementIsElementType().ToElements()
familySymbolToFind = None
for familySymbol in familyList:
    try:
        if familySymbol.LookupParameter("Имя типа").AsString() == "Кирпич":
...
if isinstance(dwg_link_instance, ImportInstance):
    ...
    imageController = ImageController(geometry, view)
    ...
    if imageController.ImagePicked:
        ...
        imageController.ColorizeFamilyInstances(familySymbolToFind)
